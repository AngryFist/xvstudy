        ...
        foreach ($postData['name'] as $key => $name){
            if($name != ''){
                if($postData['mode'][$key] == 'new'){
                    $newCategoryList[] = array('name'=>$name,'alias'=>$postData['alias'][$key],'parent'=>76,'grade'=>1,'order'=>100 * $key,'type'=>'teacher', 'mode'=>$postData['mode'][$key]);
                }else{
                    $updateCategoryList[] = array('id'=>$postData['id'][$key],'name'=>$name,'alias'=>$postData['alias'][$key],'parent'=>76,'grade'=>1,'order'=>100 * $key,'type'=>'teacher', 'mode'=>'update');
                }
            }
        }
        $retult = true;
